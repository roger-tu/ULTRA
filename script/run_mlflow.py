...
def train_and_validate(cfg, model, train_data, valid_data, device, logger, filtered_data=None, batch_per_epoch=None, tracker=None):
    ...
    if cfg.train['mlflow_tracker'] and (rank == 0):
    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        if rank == 0:
            ...
            if cfg.train.mlflow_tracker:
                ...
                mlflow.log_metric('valid_mrr', result, epoch)
                mlflow.log_metric('valid_hits-1', results['hits@1'].item(), epoch)
                mlflow.log_metric('valid_hits-3', results['hits@3'].item(), epoch)
                mlflow.log_metric('valid_hits-10', results['hits@10'].item(), epoch)
    ...
    for i,batch in enumerate(test_loader):
        ...
        if export_results:
            if world_size >1:
                ...
                h_rank_ = [torch.ones(h_ranking.size(), dtype=torch.long, device=device) for _ in range(world_size)]
                ...
                dist.all_gather(h_predictions_, h_predictions.contiguous())
                dist.all_gather(t_predictions_, t_predictions.contiguous())
                dist.all_gather(h_rank_, h_ranking)
        ...
        if rank==0:
            logger.warning(separator)
            logger.warning(f"Exporting Predictions for batch {i}") # get train edge predictions
            pl.DataFrame({
                'h': batch[:, 0].tolist(),
                'r': batch[:, 2].tolist(),
                't': batch[:, 1].tolist(),
